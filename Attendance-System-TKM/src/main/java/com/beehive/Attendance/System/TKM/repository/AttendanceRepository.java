...
public interface AttendanceRepository extends JpaRepository<Attendance, Long> {
    ...
    @Query("SELECT a FROM Attendance WHERE a.student.advisor.id = :advisorId AND a.date = :date")
    List<Attendance> findByAdvisorIdAndDate(Long advisorId, LocalDate date);

    @Query("SELECT a FROM Attendance WHERE a.student.mentor.id = :mentorId AND a.date = :date")
